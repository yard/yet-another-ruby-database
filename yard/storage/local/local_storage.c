...
int __master = 1;
// what db file name to use
char * yard_db_file_name = "yard.db";
// db directory to use
char * yard_db_dir = ".";
 ...
 */
void yard_setup_storage_settings(int cookie, int is_master, char * db_file, char * db_dir) {
  LocalCookie = cookie;
  __master = is_master;
  
  yard_db_file_name = (char *)malloc(strlen(db_file) + 1);
  memcpy(yard_db_file_name, db_file, strlen(db_file) + 1);

  yard_db_dir = (char *)malloc(strlen(db_dir) + 1);
  memcpy(yard_db_dir, db_dir, strlen(db_dir) + 1);
}
 ...
 */
VALUE pchar_to_string(char * data, long size) {
  VALUE result;
...
/*
  Loads a global variable's reference from the local storage.
*/
VALUE yard_local_load_global_variable(char * name) {
  STORAGE_DATA * data = read_data(YARD_OBJECT_SPACE_SCHEMA, string_to_key(name));
  return instantiate_object_from_data(data);
}
...
/*
    Dispatches the global variable definition message.
    
    YARD_NET_MESSAGE * message: message to process.
*/
void yard_net_dispatch_global_variable_definition(YARD_NET_MESSAGE * message) {
  char * name = (char *)message->data;  
  // message->flags should hold the length of the variable name
  void * data = (void *)((char *)message->data + message->flags + 1);
  long size = message->size - sizeof(message) - (message->flags + 1);
  
  VALUE object = rb_marshal_load(pchar_to_string(data, size));
  
  rb_gv_set((const char *)name, object);
}

/*
    Dispatches fetch request.
    
    YARD_NET_MESSAGE * message: message to process.
*/
void yard_net_dispatch_object_fetch(YARD_NET_MESSAGE * message) {
  YID * yid = (YID *)message->data;
  
  VALUE object = yard_get_object_by_yid(yid);
  VALUE marshaled_object = rb_marshal_dump(object, YARD_SHALLOW);
  
  yard_net_send_message(RSTRING_PTR(marshaled_object), 
                        RSTRING_LEN(marshaled_object), 
                        message->from, 
                        0, 
                        message->id, 
                        YARD_NET_FETCH_OBJECT_SUCCESS, 
                        0);
}
...
static void assign_to_global_variable(char * name, VALUE object) {
  if (__master) {
    ...
    yard_remote_assign_to_global_variable(name, object);      
  }
}
