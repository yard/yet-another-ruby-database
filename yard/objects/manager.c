...
/*
    Reloads the given object, fetching it from the storage and replacing it whit old version.
...
/*
    Resets the cached version of the object that corresponds to yid given.
    
    YID * yid: YID of the object to reset.
*/
void yard_reset_object_cache(YID * yid) {
  void * data = 0;
  st_delete(__yard_cache, yid, data); 
}
...
void launch_yard() {
  ...
  // assign a network dispatcher
  yard_net_initialize_network_dispatcher();
  // start network
  yard_net_initialize_server_listener();
  yard_net_initialize_clients();
